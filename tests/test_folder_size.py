...
def test_scan_community_usage_sums_per_package_and_total(tmp_path):
    ...
    result = folder_size.scan_community_usage(str(tmp_path))
...
def test_scan_community_usage_ignores_loose_files_at_top_level(tmp_path):
...
def test_scan_community_usage_sums_nested_subfolders(tmp_path):
    pkg = tmp_path / "pkg"
...
def test_scan_community_usage_missing_path_returns_empty(tmp_path):
    result = folder_size.scan_community_usage(str(tmp_path / "does_not_exist"))
    assert result == {"total_bytes": 0, "packages": []}


def test_scan_community_usage_empty_folder(tmp_path):
    result = folder_size.scan_community_usage(str(tmp_path))
    assert result == {"total_bytes": 0, "packages": []}
